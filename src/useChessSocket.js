...
const INITIAL_GAME = {
  roomId: null, myColor: null, myName: null, myRating: 1200,
  ...
  moveHistory: [], drawOffer: null, ratingChanges: null,
};
...
export function useChessSocket() {
  ...
  useEffect(() => {
    ...
    socket.on("authenticated", (data) => { setUser(data); });
    socket.on("authError", () => { localStorage.removeItem("chess_token"); localStorage.removeItem("chess_user"); setUser(null); });

    socket.on("joinedRoom", ({ roomId, color, playerName, playerRating, fen, message, timers, timeLimit, moveHistory }) => {
      setGameState((prev) => ({ ...prev, roomId, myColor: color, myName: playerName, myRating: playerRating || 1200, fen, status: "waiting", message, isSpectator: false, timers: timers || prev.timers, timeLimit: timeLimit || prev.timeLimit, moveHistory: moveHistory || [], ratingChanges: null }));
    });
    ...
    socket.on("gameStart", ({ players, fen, turn, message, timers, timeLimit, moveHistory }) => {
      setGameState((prev) => {
        const me = players.find((p) => p.socketId === socket.id);
        return { ...prev, players, fen, turn, status: "playing", message, gameOver: null, inCheck: false, lastMove: null, myColor: me ? me.color : prev.myColor, myRating: me ? me.rating : prev.myRating, timers: timers || prev.timers, timeLimit: timeLimit || prev.timeLimit, moveHistory: moveHistory || [], drawOffer: null, ratingChanges: null };
      });
    });

    socket.on("moveMade", ({ move, fen, turn, inCheck, gameOver, movedBy, timers, moveHistory, ratingChanges }) => {
      setGameState((prev) => ({ ...prev, fen, turn, inCheck, gameOver, lastMove: move, status: gameOver ? "finished" : "playing", timers: timers || prev.timers, moveHistory: moveHistory || prev.moveHistory, drawOffer: null, ratingChanges: ratingChanges || null,
    ...
    socket.on("gameRestarted", ({ players, fen, turn, message, timers, timeLimit, moveHistory }) => {
      setGameState((prev) => {
        const me = players.find((p) => p.socketId === socket.id);
        return { ...prev, players, fen, turn, status: "playing", message, gameOver: null, inCheck: false, lastMove: null, myColor: me ? me.color : prev.myColor, timers: timers || prev.timers, timeLimit: timeLimit || prev.timeLimit, moveHistory: moveHistory || [], drawOffer: null, ratingChanges: null };
      });
    });

    socket.on("gameOver", ({ type, winner, message, ratingChanges }) => {
      setGameState((prev) => ({ ...prev, status: "finished", gameOver: { type, winner }, message, ratingChanges: ratingChanges || null }));
    });
  ...
  const joinRoom = useCallback((roomId, timeLimit) => {
    socketRef.current?.emit("joinRoom", { roomId, timeLimit });
  ...
  return { connected, user, authError, authLoading, gameState, register, login, logout, joinRoom, makeMove, resign, restartGame, offerDraw, acceptDraw, declineDraw };
}
