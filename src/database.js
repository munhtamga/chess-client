...
async function createPlayer(username, passwordHash, displayName, referralCode) {
  ...
  try {
    ...
    const newPoints = parseInt(player.points) + config.REGISTER_BONUS;
...
async function claimLoginBonus(username) {
  ...
  // PostgreSQL DATE талбар ISO string эсвэл Date object хэлбэрээр ирж болно
  const lastBonus = player.last_login_bonus
    ? (typeof player.last_login_bonus === "string"
        ? player.last_login_bonus.split("T")[0]
        : player.last_login_bonus.toISOString().split("T")[0])
    : null;
  if (lastBonus === today) return { claimed: false, points: player.points };

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const newPoints = parseInt(player.points) + config.LOGIN_BONUS;
...
async function addWaitingPoints(username, minutes) {
  ...
  try {
    await client.query("BEGIN");
    const newPoints = parseInt(player.points) + amount;
    await client.query("UPDATE players SET points=$1 WHERE username=$2", [newPoints, username]);
...
async function updateRatings(whiteUsername, blackUsername, result, moves, timeLimit) {
  ...
  try {
    await client.query("BEGIN");
    const whiteNewPoints = parseInt(white.points) + whiteBonus;
    const blackNewPoints = parseInt(black.points) + blackBonus;
...
async function adjustPlayerPoints(username, amount, description) {
  ...
  const newPoints = Math.max(0, parseInt(player.points) + amount);
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("UPDATE players SET points=$1 WHERE username=$2", [newPoints, username]);
